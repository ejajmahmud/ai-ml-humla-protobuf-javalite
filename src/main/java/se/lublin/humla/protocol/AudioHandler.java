...
import static se.lublin.humla.audio.AudioUtil.calculateDecibelLevel;
...
public class AudioHandler extends HumlaNetworkListener implements AudioInput.AudioInputListener {
    ...
    public static final int FRAME_SIZE = SAMPLE_RATE/50; //SAMPLE_RATE/100;
    ...
    private HumlaService.DecibelLevelCallback inputDecibelLevelCallback;
    private HumlaService.DecibelLevelCallback outputDecibelLevelCallback;

    public void registerInputDecibelLevelCallback(HumlaService.DecibelLevelCallback callback) {
        this.inputDecibelLevelCallback = callback;
    }

    public void registerOutputDecibelLevelCallback(HumlaService.DecibelLevelCallback callback) {
        this.outputDecibelLevelCallback = callback;
        this.mOutput.registerOutputDecibelLevelCallback(callback);
    }
    ...
    public AudioHandler(Context context, HumlaLogger logger, int audioStream, int audioSource,
        ...
        mInput = null;//new AudioInput(this, mAudioSource, mSampleRate);
    ...
    public void setOutputVolume() {
        this.setOutputVolume(1.0);
    }

    public void setOutputVolume(double volume) {
        this.mOutput.setOutputVolume(volume);
    }
...
//        if (mInput != null && mInput.getSampleRate() != SAMPLE_RATE) {
//            encoder = new ResamplingEncoder(encoder, 1, mInput.getSampleRate(), FRAME_SIZE, SAMPLE_RATE);
//        }
        encoder = new ResamplingEncoder(encoder, 1, 22050, FRAME_SIZE, SAMPLE_RATE);
    ...
    public void onAudioInputReceived(short[] frame, int frameSize) {
        ...
        // Calculate decibels
        if(this.inputDecibelLevelCallback != null) {
            this.inputDecibelLevelCallback.decibelLevelEvent(calculateDecibelLevel(frame, frameSize));
        }
