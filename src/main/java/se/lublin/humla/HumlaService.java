...
public class HumlaService extends Service implements IHumlaService, IHumlaSession, HumlaConnection.HumlaConnectionListener, HumlaLogger, BluetoothScoReceiver.Listener {
    ...
    private DecibelLevelCallback inputDecibelLevelCallback;
    private DecibelLevelCallback outputDecibelLevelCallback;

    public void registerInputDecibelLevelCallback(DecibelLevelCallback callback) {
        this.inputDecibelLevelCallback = callback;
        if (this.mAudioHandler != null)
            this.mAudioHandler.registerInputDecibelLevelCallback(callback);
    }

    public void registerOutputDecibelLevelCallback(DecibelLevelCallback callback) {
        this.outputDecibelLevelCallback = callback;
        if(this.mAudioHandler != null)
            this.mAudioHandler.registerOutputDecibelLevelCallback(callback);
    }
    ...
    public void setOutputVolume(double volume) {
        this.volume = volume;
        // Only set the volume of the audio handler now if it has actually been instantiated already
        // Else, the volume will get set the next time it gets initialized when a connection is established
        if(mAudioHandler != null) {
            mAudioHandler.setOutputVolume(this.volume);
        }
    }
...
//            auth.addCeltVersions(Constants.CELT_11_VERSION);
    ...
    public void onConnectionSynchronized() {
        ...
        try {
            mAudioHandler = mAudioBuilder.initialize(
                    ...
                    mVoiceTargetId);
            this.registerInputDecibelLevelCallback(this.inputDecibelLevelCallback);
            this.registerOutputDecibelLevelCallback(this.outputDecibelLevelCallback);
            mAudioHandler.setOutputVolume(this.volume); // Set initial volume to whatever our setting is (can be changed dynamically later thru setOutputVolume)
    ...
    public void onAudioInputReceived(short[] frame, int frameSize) {
        if(mAudioHandler != null)
            mAudioHandler.onAudioInputReceived(frame, frameSize);
    }
    ...
    public interface DecibelLevelCallback {
        void decibelLevelEvent(double decibels);
    }
}
